...
void Part1(void)
{
    ...
    while(sym < 126) //Iterate over each character from '!' through '`'
    {
        Print_To_LCD(Clear1); //Make sure the top half of the LCD is clear
        strcpy(Str_1, Clear1); //Clear out Str_1
        __delay_ms(500);
        for(i = 1;i <= 8;i++,sym++) //For loop to populate Str_1
        {
            Str_1[i] = sym;
            Print_To_LCD(Str_1);
            __delay_ms(500);
        ...
        Print_To_LCD(Clear2); //Make sure the bottom half of the LCD is clear
        strcpy(Str_2, Clear2); // Clear out Str_2
        for(i = 1;i <= 8;i++,sym++) // For loop to populate Str_2
        {
            Str_2[i] = sym;
            Print_To_LCD(Str_2);
            __delay_ms(500);
...
{
    strcpy(Str_1, Clear1); //Clear out Str_1
    strcpy(Str_2, Clear2); //Clear out Str_1
    __delay_ms(1000);
    //Setting the character arrays to contain and display the first equation
    Str_1[1] = 'X';
    Str_1[2] = '=';
    Str_2[1] = 0xe0;
    Str_2[2] = '*';
    ...
    Print_To_LCD(Str_2);
    __delay_ms(5000);
    //Setting the character arrays to contain and display the second equation
    Str_1[1] = 'Y';
    Str_1[2] = '=';
    Str_2[1] = 's';
    Str_2[2] = 'i';
...
void Part3(void)
{
    char custom[] = {0x58, 0x80, 0x80, 0x89, 0x95, 0x95, 0x92, 0x80,0x80, 0}; //Setting the pixels to shade in
    Print_To_LCD(custom);
    char top[] = {0x80,' ', ' ', ' ', ' ', 0x03, ' ', ' ', ' ', 0}; //Make tilde sit in the middle of the character array
    Print_To_LCD(top); //Display on LCD
    __delay_ms(5000);
}

void Part4(void)
{
    char custom[] = {0x58, 0x80, 0x84, 0x84, 0x9f, 0x8e, 0x8e, 0x8e,0x91, 0}; //Setting pixels to shade in
    Print_To_LCD(custom);
    char top[] = {0x80,' ', ' ', ' ', ' ', 0x03, ' ', ' ', ' ', 0}; //Make turtle sit in the middle of the top array
    char bottom[] = {0xc0,' ', ' ', ' ', ' ', 0x03, ' ', ' ', ' ', 0};//Make turtle sit in the middle of the bottom array
    Print_To_LCD(top); //Display turtle on top
    __delay_ms(500);
    Print_To_LCD(Clear1);//Clear turtle from top
    Print_To_LCD(bottom);//Print turtle on bottom
    __delay_ms(500);
    Print_To_LCD(Clear2);//Clear turtle from the bottom
}
